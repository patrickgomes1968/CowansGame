...
namespace CowansGameLibrary
{
    public class CowansGame
    {
        ...
        public static void RecalculateLives()
        {
            ...
            for(int i = 0; i < total; i++)
            {
                ...
                grid[p.x,p.y] = 0; //reset
                if (LiveOrDie(grid[p.x,p.y],liveNeighs)) grid[p.x,p.y] = 1 ;
                
                //Console.WriteLine($"x = {p.x} and y = {p.y}, val is {grid[p.x,p.y]} and Live Neighbours = {liveNeighs}");	
            }
        }   

        public static Boolean LiveOrDie(int state, int liveNeighs) 
        {
            Boolean live = false;
            switch (state)
            { 
                case 1: 
                    if (liveNeighs == 2 || liveNeighs == 3 ) live = true; //kill
                break;
                case 0:
                    if (liveNeighs == 3) live = true;
                    break;
            }
            return live;
        }     
